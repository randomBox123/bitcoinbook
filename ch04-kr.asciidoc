...
((("cryptography", "defined")))((("cryptography", see="also keys and addresses")))비트코인이 _암호학 (cryptography)_ 에 기반하고 있다는 것을 들어본 적 있을 것이다. 암호학은 컴퓨터 보안에서 광범위하게 사용되는 수학의 한 분야이다. 암호학은 그리스어로 "비밀 문서 (secret writing)"을 의미하지만 과학에서 암호학은 암호화라고 하는 비밀 문서 이상의 것을 의미한다. 또한 암호학은 비밀을 누설하지 않고 비밀에 대한 지식을 증명하거나 (디지털 서명) 데이터의 진위성 (디지털 지문)을 증명하는 데 사용된다. 이러한 유형의 암호학 증명들은 비트코인에서 중요한 수학적 도구 이며 비트코인 어플리케이션에 널리 사용된다.
((("encryption")))((("encryption", see="also keys and addresses")))통신 데이터와 거래 데이터는 암호화되지 않고 자산을 보호하기 위해 암호화 될 필요도 없으므로, 암호화는 역설적으로 비트코인에서 중요한 부분은 아니다. 이 장에서 키, 주소 및 지갑의 형태로 자금의 소유권을 제어하기 위해 비트코인에서 사용되는 일부 암호학에 대해 살펴볼 것이다.
...
((("digital keys", see="keys and addresses")))((("keys and addresses", "overview of", id="KAover04")))((("digital signatures", "purpose of")))비트코인의 소유권은 _디지털 키 (digital keys)_, _비트코인 주소 (bitcoin addresses)_ 및 _디지털 서명 (digital signatures)_ 을 통해 성립된다. 디지털 키는 실제로 네트워크상에 저장되지 않지만 대신 _지갑 (wallet)_ 이라는 파일이나 단순한 데이터베이스로 사용자에 의해 생성되거나 저장된다. 사용자 지갑의 디지털 키는 비트코인 프로토콜과는 완전히 독립되어 있고, 블록체인을 참조하거나 인터넷에 접속하지 않아도 사용자의 지갑 소프트웨어로 생성하고 관리할수 있다. 키는 탈중앙화된 신뢰 및 제어, 소유권 입증 및 암호 증명 (cryptographic-proof) 보안 모델을 포함한 흥미로운 비트코인의 다수 특성들을 가능하게 한다.
...
대다수 비트코인 거래는 블록체인에 포함되기 위해 유효한 서명이 필요하며 서명은 비밀키가 있어야만 생성될 수 있다; 그러므로 이것의 복사본만 가지고 있으면 비트코인을 통제할 수 있다. ((("witnesses")))자금을 소비하기 위해 사용되는 디지털 서명은 암호학에서 사용되는 용어로 _증인 (witness)_ 이라고도 한다. 비트코인 거래에서 증인 데이터는 사용된 자금의 실제 소유권을 증명한다.
...
키는 개인 (비밀)키와 공개키 한 쌍으로 구성된다. 공개키는 은행 계좌번호와 유사하고 비밀키는 계좌를 관리할 수 있는 비밀 PIN 또는 수표의 서명과 유사하다고 생각하면 된다. 비트코인의 사용자들은 이러한 비밀키를 거의 볼 수 없다. 대부분의 경우 비밀키는 지갑 파일 내에 저장되고 비트코인 지갑 소프트웨어에 의해 관리된다.
...
비트코인 거래에 있어서 수취인의 공개키는 _비트코인 주소_ 라고 하는 디지털 지문으로 표현되며, 이는 수표에 수취인의 이름을 기입하는 것 (즉, "~에게 지불")과 동일한 방식이다. 대부분의 경우 비트코인 주소는 공개키로부터 생성되며 공개키에 대응한다. 하지만 모든 비트코인 주소가 공개키를 나타내는 것은 아니다; 이후에 살펴보겠지만 스크립트 등이 공개키 역할을 하기도 한다. 이러한 방식으로 비트코인 주소는 수표처럼 수취를 요약해서 보여줌으로써 거래 송금처를 유연하게 지정할 수 있다. 즉, 단 하나의 지급수단으로도 개인 계좌나 기업 계좌, 청구서에 대한 지불 또는 현금 거래 모두 가능하다. 비트코인 주소는 사용자가 일상적으로 보게 될 키를 대신하는 유일한 수단이며, 이것은 다른 사람들과 공유해야할 부분이기 때문이다.
...
먼저 암호학에 대해 소개하고 비트코인에 사용되는 수학을 설명할 것이다. 다음 키가 어떻게 생성 및 저장, 관리되는지에 대해 알아볼 것이다. 그리고 개인키, 공개키, 주소, 스크립트 주소 등을 대신하기 위해 사용되는 다양한 인코딩 형식에 대해 살펴볼 것이다. 마지막으로 vanity 주소, 다중서명 (multisignature) 및 스크립트 주소와 종이 지갑 (paper wallet) 등 키와 주소의 특별한 사용에 대해 다룰 것이다.
...
((("keys and addresses", "overview of", "public key cryptography")))((("digital currencies", "cryptocurrency")))공개키 암호법은 1970년대에 발명되었으며 컴퓨터 및 정보보안의 수학적 기초이다.
...
공개키 암호법이 발명된 이후 소수 지수함수 및 타원곡선 곱셈함수 (elliptic curve multiplication) 등 몇 가지 적합한 수학 함수가 발견되었다. 이러한 수학 함수들은 실제로는 비가역적이며, 이는 한 방향으로 계산하는 것은 쉽지만 반대 방향으로 계산하는 것은 불가능하다는 것을 의미한다. 이러한 수학 함수에 기반한 암호법은 디지털 비밀번호 및 위조가 불가능한 디지털 서명의 생성을 가능하게 한다. 비트모인은 공개키 암호법으로 타원곡선 곱셈함수를 기본으로 사용한다.
...
비트코인에서는 비트코인에 접근하는 권한을 주는 한 쌍의 키를 생성하기 위해 공개키 암호법을 사용한다. 한 쌍의 키는 개인키와--개인키에서 파생된--고유의 공개키로 구성된다. 공개키는 비트코인을 전송받을 때 사용되고 개인키는 비트코인을 소비하는 거래에 서명할 때 사용된다.
...
공개키와 개인키 사이에는 수학적 관계가 성립하며, 이 때문에 메시지에 대한 서명을 할때 개인키를 사용할 수 있다. 이 서명은 개인키를 노출하지 않고 공개키만으로 유효성 확인이 가능하다.
...
비트코인을 소비하는 경우, 현재 비트코인 소유주가 이 비트코인을 소비하기 위해서 거래 내에 공개키 및 서명 (매번 다르지만 동일한 개인키로부터 생성됨)을 공개한다. 공개키 및 서명을 공개함으로써 비트코인 네트워크 상 모든 사람들이 그 거래를 검증하고 유효한 거래로 받아들일 수 있게 된다. 동시에 비트코인을 전송하려고 하는 사람이 전송 당시에 비트코인을 소유하고 있었는지도 확인 할 수 있다.
...
((("keys and addresses", "overview of", "key pairs")))대부분의 지갑 구현에서 개인키 및 공개키는 편의를 위해 _한 쌍_ 의 형태로 저장된다. 그러나 공개키는 개인키로부터 계산할 수 있기 때문에 개인키만 저장하는 것 역시 가능하다.
====
