...
* ((("exchange rates", "listing services")))당신의 은행 계좌와 연계된 비트코인 통화 거래소를 사용하자. 현재 많은 국가에 판매자와 구매자가 현지 통화로 비트코인을 교환할 수 있는 시장을 제공하는 통화 거래소가 존재한다. https://bitcoinaverage.com[BitcoinAverage]와 같은 환율 목록 서비스는 주로 각 통화에 대한 비트코인 환율 목록을 제공한다.
...
((("privacy, maintaining")))((("security", "maintaining privacy")))((("digital currencies", "currency exchanges")))((("currency exchanges")))((("digital currencies", "benefits of bitcoin")))((("bitcoin", "benefits of")))다른 결제 시스템과 비교하여 비트코인의 장점 중 하나는 올바르게 사용한다면 사용자에게 더 큰 프라이버시를 제공한다는 것이다. 비트코인을 취득, 보유 및 지출하는 경우에도 민감하며 개인적으로 식별가능한 정보를 제 3자에게 노출할 필요가 없다. 그러나 비트코인이 통화 거래소와 같은 기존 시스템을 거치게 되면 주로 국가 및 국제적인 규정이 적용된다. 당신의 자국 통화로 비트코인을 교환하려면 신원 정보 및 은행 정보를 제공해야 할 것이다. 사용자는 비트코인 주소가 신원에 연동된 후에는 비트코인 거래와 연관된 모든 정보는 쉽게 확인되고 추적될 수 있음을 인지해야 한다. 이것이 바로 많은 사용자가 그들의 지갑에 교환 계정을 연결하지 않기로 선택한 이유 중 하나이다.
====
...
앨리스는 친구가 비트코인에 대해 알려 주었기 때문에 첫 번째 비트코인을 취득하는 쉬운 방법이 있다. 다음에는 앨리스가 친구 조로부터 비트코인을 구매하는 방법과 조가 앨리스의 지갑으로 비트코인을 전송하는 방법에 대해 살펴볼 것이다.
...
((("getting started", "exchange rates")))((("exchange rates", "determining")))앨리스가 조에게서 비트코인을 구매하기 전에, 그들은 비트코인과 달러간 _환율_ 을 정해야 한다. 비트코인을 사용하는 새로운 사용자들은 다음과 같은 질문을 떠올릴 것이다: "누가 비트코인 가격을 정하는 것이지?" 이 질문에 대한 간단한 대답은 시장에 의해 가격이 결정된다는 것이다.
...
((("exchange rates", "floating")))((("floating exchange rate")))대다수 화폐와 유사하게 비트코인은 _변동 환율제 (floating exchange rate)_ 이다. 즉, 다른 통화에 대한 비트코인의 가치는 비트코인이 거래되는 다양한 시장의 수요와 공급에 따라 변동된다. 예를 들어, 달러로 교환되는 비트코인의 "가격"은 비트코인과 달러의 가장 최근 거래를 기반으로 하여 각 시장에서 계산된다. 따라서 가격은 초당 몇 번씩 미세하게 변동되는 경향이 있다. 가격 서비스는 여러 시장의 가격을 합산하고 통화 조합 (즉, BTC/USD)의 다수 시장의 환율을 나타내는 볼륨 가중 평균을 계산한다.

// There are hundreds of applications and websites that can provide the current market rate. Here are some of the most popular:
현재 시장 환율을 제공하는 많은 어플리케이션과 웹사이트가 있다. 다음은 이들 중 가장 유명한 곳을 나열한 것이다:
...
http://bitcoinaverage.com/[Bitcoin Average]:: ((("BitcoinAverage")))각 통화에 대한 볼륨 가중 평균에 대해 간단하게 제공하는 사이트
...
http://coincap.io/[CoinCap]:: 비트코인을 포함하여 다수의 암호화폐에 대한 환율과 시장 점유율을 제공하는 서비스
...
http://bit.ly/cmebrr[Chicago Mercantile Exchange Bitcoin Reference Rate]::CME의 투자 데이터 피드의 일부로 제공되는 제도적 및 계약상의 참조에 사용될 수 있는 참조 비율
...
이러한 다양한 사이트 및 어플리케이션 뿐만 아니라 대다수 비트코인 지갑은 자동적으로 비트코인과 다른 통화 사이를 변환해 준다. 조는 앨리스에게 비트코인을 전송하기 전에 그의 지갑을 사용하여 자동적으로 가격을 변환할 것이다.
...
((("getting started", "sending and receiving bitcoin", id="GSsend01")))((("spending bitcoin", "bitcoin wallet quick start example")))((("spending bitcoin", see="also transactions")))앨리스는 10달러만 비트코인으로 환전하기로 결정했고, 이 새로운 기술에 많은 돈을 투자하는 것은 위험하다고 생각했다. 그녀는 조에게 10달러를 지불했고 Mycelium 지갑을 열고 수신버튼을 눌렀다. 이 어플리케이션에서는 앨리스의 최초의 비트코인 주소와 함께 QR코드를 표시했다.

// Joe then selects Send on his smartphone wallet and is presented with a screen containing two inputs:
그 후 조는 그의 스마트폰 지갑에서 전송 버튼을 눌렀고 다음의 두 가지를 입력하라는 화면이 나타났다:

// * A destination bitcoin address
* 입금될 비트코인 주소
// * The amount to send, in bitcoin (BTC) or his local currency (USD)
* 비트코인 (BTC) 또는 자국 화폐 (USD)로 전송할 금액
...
비트코인 주소를 입력하는 공간에는 QR코드처럼 보이는 작은 아이콘이 있다. 이것은 조가 스마트폰 카메라로 바코드를 스캔할 수 있게 해주므로, 조는 길고 타이핑하기 어려운 앨리스의 비트코인 주소를 하나씩 입력하지 않아도 된다. 조는 QR코드 아이콘을 눌러 스마트폰 카메라를 활성화 시키고 앨리스의 스마트폰에 표시도니 QR코드를 스캔한다.
...
조는 이제 앨리스의 비트코인 주소를 수신자로 설정했다. 조는 10 달러 금액을 입력하고 그의 지갑은 온라인 서비스에서 가장 최근의 환율을 활용하여 비트코인으로 변환한다. 그 당시의 1 비트코인은 100 달러였고, 10달러는 0.10 비트코인 (BTC) 또는 조의 지갑 스크린샷에 보이는 것처럼 100 밀리비트코인 (mBTC)에 해당한다 ( <<airbitz-mobile-send>>를 참고하라).

[[airbitz-mobile-send]]
[role="smallereighty"]
// .Airbitz mobile bitcoin wallet send screen
.Airbitz 모바일 비트코인 지갑 전송 화면
...
송금 시 실수가 있으면 돌이킬 수 없기 때문에 조는 정확한 금액을 입력했는지 주의를 기울여 확인한다. 비트코인 주소와 금액을 여러번 확인한 후 조는 거래를 실행하기 위해 전송 버튼을 누른다. 조의 모바일 비트코인 지갑에서 앨리스가 제공한 주소로 0.10 비트코인을 전송하는 거래가 성립되면서, 지갑에 있던 금액이 빠져나가고 조의 개인키로 거래에 서명하게 된다. 이를 통해 조가 앨리스의 새 주소로 송금을 허가했다는 것을 비트코인 네트워크에 알리게 된다. 거래가 P2P 프로토콜을 통해 이루어졌기 때문에 해당 거래는 신속하게 비트코인 네트워크로 전파된다. 1초도 되지 않아 네트워크에 연결된 노드 대부분은 거래를 수신하고 최초로 앨리스의 주소를 확인한다.
...
한편 앨리스의 지갑은 그녀의 지갑 주소와 일치하는 것을 찾으며 지속적으로 비트코인 네트워크에서 게시되는 거래를 "청취 (listening)"한다. 조의 지갑이 거래를 전송하고 몇 초 후에 앨리스의 지갑에 0.10 BTC가 수신되고 있음을 알 수 있다.
...
((("getting started", "confirmations")))((("confirmations", "bitcoin wallet quick start example")))((("confirmations", see="also mining and consensus; transactions")))((("clearing", seealso="confirmations")))처음에는 앨리스의 주소에 조와의 거래가 "미승인 (Unconfirmed)"이라고 나올 것이다. 이는 해당 거래가 네트워크로 전파되었지만 블록체인이라는 비트코인 거래 장부에는 아직 포함되지 않았음을 의미한다. 승인되기 위해서는 거래는 블록에 반드시 포함되어야 하며 블록체인에 추가되어야 한다. 이는 평균적으로 매 10분마다 발생한다. 전형적인 경제 용어로는 _클리어링 (clearing)_ 이라고 한다. 비트코인 거래의 전파, 입증 및 클리어링 (승인)에 대한 더 자세한 내용은 <<mining>> 을 참고하라.
****
...
앨리스는 이제 소비 가능한 0.10 BTC를 보유한 자랑스러운 소유주가 되었다. 다음 챕터에서는 비트코인을 사용한 첫 구매를 살펴보고, 거기에 해당하는 거래 및 전파 기술에 대해 좀 더 자세히 알아 볼 것이다.((("", startref="BCbasic01")))((("use cases", "buying coffee", startref="aliceone")))
