...
다음 섹션에서는 이 거래에 대해 더 자세하게 살펴볼 것이다. 앨리스의 지갑이 어떻게 거래를 성사시키는지, 거래가 네트워크를 통해 어떻게 전파되는지, 해당 거래가 어떻게 검증되는지, 그리고 마지막으로 Bob이 전송받은 금액을 차후 거래에 어떻게 사용할 수 있는지에 대해 알아 볼 것이다.
